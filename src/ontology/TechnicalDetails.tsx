/**
 * 技术详情抽屉（Batch 3.5 · 统一页面头部的次级信息入口）。
 *
...
function TechnicalDetailsContent({ctx, onClose}: {ctx: ModelContextValue; onClose: () => void}) {
  ...
  return (
    <Drawer
      title="技术详情"
      subtitle="当前页面与视图的工程标识（原始值，来自 HTTP 返回）。产品界面中的状态带只保留产品语义。"
      onClose={onClose}
      width="max-w-[560px]"
    >
      <div className="space-y-5 text-[12px]">
        ...
        <Section title="视图引用（URL 为唯一事实来源）">
          <MonoRow label="modelId" value={ctx.modelId}/>
        ...
        {/* 内容标识 */}
        <Section title="内容标识">
          <MonoRow label="contentHash" value={ctx.resolvedView?.contentHash ?? '…'} full/>
          <MonoRow label="ETag (If-Match)" value={ctx.resolvedView?.etag ?? '…'} full/>
        ...
        <Section title={`API Path（当前功能：${ctx.tab}）`}>
          {TAB_API_PATHS[ctx.tab].map((p) => (
            <p key={p} className="font-mono text-[10.5px] text-slate-600 break-all">{p}</p>
          ))}
          {ctx.isDraft && <p className="font-mono text-[10.5px] text-slate-500 break-all pt-1 border-t border-slate-100 mt-1">{WRITE_PATH}</p>}
        </Section>
          ...
          <div className="flex flex-wrap gap-1.5">
            {ctx.capabilities.length > 0
              ? ctx.capabilities.map((c) => (
                <span key={c} className="px-1.5 py-0.5 rounded border text-[10px] font-mono bg-slate-50 text-slate-600 border-slate-200">{c}</span>
              ))
              : <span className="text-slate-400">读取中…</span>}
          </div>
          <p className="text-[10.5px] text-slate-400 mt-1.5">actorId {ctx.actorId}</p>
        </Section>
          ...
          {registry && (
            <div className="space-y-1.5">
              ...
              <details className="pt-1.5">
                <summary className="text-[11px] font-semibold text-slate-500 cursor-pointer">原始 JSON（implementations · workflows · ioContracts）</summary>
                <pre className="mt-1.5 p-3 bg-slate-50 border border-slate-200 rounded-lg text-[9.5px] font-mono overflow-x-auto max-h-72 overflow-y-auto">{JSON.stringify({
...
function MonoRow({label, value, full}: {label: string; value: string; full?: boolean}) {
  return (
    <div className={`flex items-start gap-3 ${full ? '' : 'justify-between'}`}>
      <span className="text-[10.5px] text-slate-400 font-semibold shrink-0">{label}</span>
      <span className={`font-mono text-[10.5px] text-slate-600 break-all ${full ? 'flex-1' : 'text-right'}`}>{value}</span>
    </div>
  );
}
